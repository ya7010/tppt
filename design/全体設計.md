...
## スライドマスター
スライドマスターはファイルを読み込む際に作られる。

```python
from pptxr import SlideMaster

from pptxr import Presentation

presentation = (
    Presentation.builder(SlideMaster("slide_master.pptx"))
    .slide(
        TitleSlide(title="Hello, world!").builder()
        .text("Hello, world!", x=(100, "pt"), y=(100, "pt"))
        .image(path="image.png", width=(100, "pt"), height=(100, "pt")),
...
class MySlideMaster(SlideMaster[MySlideTemplate]):
    def __init__(self) -> None:
        super().__init__(__file__ /"slide_master.pptx")

# ここでは、MySlideTemplate を継承したスライドテンプレートを作成する

presentation = (
    Presentation.builder(slide_master=MySlideMaster)
    .slide(
        # MySlideTemplate を継承したスライドテンプレートしか代入できない
